...
class IOUSBDetector {
    ...
    //private let ioRemoveCallback : IOServiceMatchingCallback?
...
//            self.callbackQueue?.async {
                    usbkeyRemoveCtl() // runs anonymous callback functions
...
//            }
    ...
    init? ( vendorID: Int, productID: Int ) {
        ...
        self.callbackQueue = DispatchQueue.global(qos: DispatchQoS.QoSClass.background)
        
        
        // Setting up the DASession to detect insertion of usb device
        queueDA = DispatchQueue.global(qos: DispatchQoS.QoSClass.default) /*(label: "IODADetector")*/
    ...
    func startDetection ( ) -> Bool {
        ...
        let diskcallback : DADiskDescriptionChangedCallback = {
            (disk, array, context) in
                ...
                let volumeIndex : String = volumeArray[0] as! String
            
                if let volumePath = (diskDict as! NSDictionary)[volumeIndex] as! URL?
                {
         ...
         */
        let ioRemoveCallback : IOServiceMatchingCallback? /*ioRemoveCallback*/ = {
        ...
        self.dispatchEvent(iterator: removedIterator)
...
func ejectHelper (diskPath : String, dadisk: DADisk?)
{
    ...
    if #available(OSX 10.12, *)
    {
        os_log("diskutl eject %s", log: OSLog.default,
               type: .info, diskPath)
    }
    else
    {
