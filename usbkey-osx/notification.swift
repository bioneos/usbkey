    ...
    func startDetection ( ) -> Bool {
        ...
        self.fsEventCallback =
            { (streamRef, clientCallBack, numEvents, eventPaths, eventFlags, eventIds) in
                ...
                usbkey_InsertCtl(keyPath: "/Library/usbkey/key", diskPath: usbPath)
 ...
 */
func usbkey_removeCtl (usbkey_root: String = "/Library/usbkey/"){
    let fileManager : FileManager = FileManager.default
    let homeDir = fileManager.homeDirectoryForCurrentUser //genetic home directory
    let fullUSBRoot : String = homeDir.path + usbkey_root
    
    // checks if INSERT file exist if not error will occur
    let (_, file) = checkFileDirectoryExist(fullPath: fullUSBRoot + "INSERTED")
    if (!file){
        return
    }
    else{
        do {
            try fileManager.removeItem(at: URL(fileURLWithPath: fullUSBRoot + "INSERTED"))
        }
        catch {
            // fails if file did exist but suddenly disappears
            print ("Error - INSERT file located at " + fullUSBRoot + " Disappeared") //add log
            return
        }
    }
    // goes to sleep mode
 ...
 */
func usbkey_InsertCtl(keyPath: String, diskPath : String?, usbkey_root : String = "/Library/usbkey/"){
    ...
    if (!directory){
        do {
            ...
            print ("Directory" + keyPath + " couldn't be created") //add log and display
            return
        }
    }
    ...
    decryptImage(path: homeDir.path + keyPath, sparsePath: diskPath!)
    ...
    /*let (_, file) = checkFileDirectoryExist(fullPath: homeDir.path + usbkey_root + "EJECT")
    if (file){
